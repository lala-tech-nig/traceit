                ...
                <div className="flex-1 py-10 px-6 space-y-4 overflow-y-auto">
                    ...
                    {user.role === 'admin' && (
                        <>
                            ...
                            <Link to="/admin" className={`group relative flex items-center gap-4 px-6 py-4 rounded-2xl transition-all duration-300 font-bold text-base ${
                    ...
                    {user.role === 'technician' && (
                        <>
                            ...
                            <Link to="/dashboard/subscription" className={`group relative flex items-center gap-4 px-6 py-4 rounded-2xl transition-all duration-300 font-bold text-base ${
                ...
                <header className="md:hidden bg-white border-b border-neutral-200 p-4 flex items-center justify-between z-10">
                    ...
                    <div className="flex items-center gap-2">
                        {user.role === 'basic' && user.isApproved && user.verificatorStatus !== 'approved' && (
                            <Link to="/dashboard/verificator-apply" className="bg-primary/10 text-primary px-3 py-2 rounded-xl flex items-center gap-1.5 transition-colors whitespace-nowrap">
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-neutral-200 z-[100] px-2 pt-2 pb-[max(env(safe-area-inset-bottom),_0.75rem)] shadow-[0_-15px_40px_rgba(0,0,0,0.08)]">
                <div className="flex justify-between items-center max-w-sm mx-auto relative h-16 w-full">
                    {[
                        ...
                        return (
                            <Link to={tab.href} key={tab.href} className="relative flex flex-col items-center justify-center flex-1 h-full z-10 select-none -webkit-tap-highlight-transparent group">
                                {isActive && (
                                    <motion.div
