...
export default function TransfersPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            const [transfersRes, devicesRes] = await Promise.all([
                ...
                <div className="bg-white border border-neutral-200 p-6 md:p-8 rounded-3xl shadow-sm mb-8">
                    ...
                    <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleInitiate}>
                        ...
                        <div className="md:col-span-2 flex justify-end">
                            <button disabled={initiating || myDevices.length === 0} type="submit" className="bg-foreground text-white px-8 py-3 rounded-xl font-bold hover:bg-neutral-800 transition-colors disabled:opacity-70">
                                {initiating ? 'Initiating...' : 'Send Transfer Request'}
                            </button>
                        </div>
                    </form>
                </div>
